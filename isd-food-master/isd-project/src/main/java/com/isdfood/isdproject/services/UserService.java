...
public class UserService extends PersonService {
    ...
    //UserProfileRefactor
    public void profileRefactor(Long id,String email,String firstName,String lastName,String skypeId){
        User user = getUserRepository().findById(id).orElseThrow();
        user.setEmail(email);
    ...
    public void profileRefactor(Long id,String email,String firstName,String lastName,String skypeId,char[] password,Byte[] img) throws IdenticalPasswordException {
        User user = getUserRepository().findById(id).orElseThrow();
        user.setEmail(email);
        user.setFirstName(firstName);
        user.setLastName(lastName);
